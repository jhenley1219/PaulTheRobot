...
The study's text content lives in the `study_materials/` directory and is loaded by `AnthroGUI.py` at runtime:

*   `study_materials/CONSENT.txt`: The consent form for the research study, displayed to the user at the beginning of the experiment.
*   `study_materials/FRAMING_anthro.txt`: An anthropomorphic description of the robot "Paul." Used in one of the experimental conditions.
*   `study_materials/FRAMING_tech.txt`: A technical description of the robot. Used in the other experimental condition.
*   `study_materials/INSTRUCTIONS.txt`: Instructions for the user on how to perform the PCB inspection task.
*   `study_materials/STORY.txt`: A narrative background story for the user to read before starting the task.
*   `study_materials/IDAQ instructions`: Reference text for the Individual Differences in Anthropomorphism Questionnaire (IDAQ).
